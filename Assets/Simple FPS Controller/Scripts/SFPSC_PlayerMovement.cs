...
public class SFPSC_PlayerMovement : MonoBehaviour
{
    ...
    {
        if (
            (wallRun != null && wallRun.IsWallRunning)
        ...
        else
        {
            ...
            isGrounded =
                (Mathf.Abs(rb.velocity.y - prevY) < .1f)
        ...
        if (!enableMovement)
            return;
        inputForce =
            (transform.forward * vInput + transform.right * hInput).normalized
            * (Input.GetKey(SFPSC_KeyManager.Run) ? runSpeed : walkSpeed);
        ...
        if (isGrounded)
        {
            ...
            rb.velocity = Vector3.Lerp(
                rb.velocity,
                inputForce,
                changeInStageSpeed * Time.fixedDeltaTime
            );
        }
        else
            // Air control
            rb.velocity = ClampSqrMag(
                rb.velocity + inputForce * Time.fixedDeltaTime,
